...
/**
 * Displays a list of subscription plans in a table format.
 *
 * Features:
 * - Shows plan pricing, limits, and status.
 * - Allows toggling plan active/inactive state.
 * - Provides an edit action for each plan.
 * - Displays a fallback message when no plans are available.
 *
 * @param {SubscriptionPlansListProps} props  Component props.
 * @param {SubscriptionPlan[]} props.plans Array of subscription plans to display.
 * @param {(plan: SubscriptionPlan) => void} props.onEdit  Callback to edit a plan.
 * @param {(plan: SubscriptionPlan) => void} props.onToggle  Callback to toggle plan status.
 * @returns {JSX.Element} Rendered subscription plans table or empty state message.
 */
const SubscriptionPlansList = ({ plans, onEdit, onToggle }: SubscriptionPlansListProps): JSX.Element => {
  if (plans.length === 0) {
        ...
        <tbody>
          {plans.map((plan: SubscriptionPlan) => (
            <tr key={plan.id}>
              ...
              <td>{plan.scans_per_month}</td>
              <td>{plan.landscape_gens_per_month}</td>
              <td>
