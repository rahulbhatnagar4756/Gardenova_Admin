...
/**
 * Sidebar component that handles navigation, token validation, and logout logic.
 *
...
export const Sidebar = () => {
   ...
   *
   * @returns {boolean} True if the token is valid and belongs to an admin; otherwise false.
   */
  const isTokenValid = () => {
  ...
  /**
   * Handles the logout action when clicking the logout link.
   *
  ...
  /**
   * Validates token before allowing navigation to protected routes.
   *
   * @param {React.MouseEvent<HTMLAnchorElement>} e Click event.
   * @returns {void}
   */
  ...
  /**
   * Closes the sidebar by removing the expanded CSS class.
   *
   * @returns {void}
   */
  const handleClose = () => {
